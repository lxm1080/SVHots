                               ...
                               sig_cols = mycol, plot = TRUE, resdir = resdir)
{
  ...
  for (s in unique(vcf[, "Sample"])) {
    ...
    #      border = sig_cols[rownames(prop.sigs.)])
      # pie(t(prop.sigs.), labels = rownames(prop.sigs.),
           # main = paste("Mutational Signatures Contribution in: ", colnames(prop.sigs.), sep = ""), col = sig_cols[rownames(prop.sigs.)],
          # border = sig_cols[rownames(prop.sigs.)])
      dev.off()
    }
...
#' @param method Specification of the NMF algorithm. â€˜brunetâ€? method corresponds to the standard NMF algorithm from Brunet,2004
