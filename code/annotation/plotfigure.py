...
def plotratio(df,path):
    for i in range(len(df)):
        ...
        ax1.set_title(title, fontsize=33)
        ...
        ax1.set_ylabel("density per MB",fontsize=26)#纵坐标label
...
def annotate_heatmap(im,pvalue, data=None, valfmt="{x:.2f}",
    ...
    **kwargs
    ...
    for i in range(data.shape[0]):
        for j in range(data.shape[1]):
            if pvalue[i,j]<0.001 and data[i, j]>=1.5:
                kw.update(color='red')
...
def plot_OR_comparison(typelist):
    data = pd.DataFrame(columns=['element name']) #data是存OR的
    data_p = pd.DataFrame(columns=['element name']) #data_p是存p的
    
    for i in range(len(typelist)):        
        temp=pd.read_csv('Results/'+typelist[i]+'/cancer_specifity_statistic.csv')
        if i==0:
            data['element name']=temp['element name']
            data_p['element name']=temp['element name']
        data[typelist[i]]=temp['OR']
        data_p[typelist[i]]=temp['pvalue(poisson test)']
    #纵坐标
    element_name=[]
    ...
    pvalue=[]
    for i in range(len(data_p.index)):#data每一行
        temp=[]
        row=data_p.loc[i]
        for j in range(1,len(data_p.columns)):
    ...
    im, cbar = heatmap(OR, element_name, typename, ax=ax,cmap="YlGn", cbarlabel="OR")
    
    texts = annotate_heatmap(im,pvalue)#, valfmt="{x:.1f} t")
